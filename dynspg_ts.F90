...
!AW v421 update      REAL(wp), ALLOCATABLE, DIMENSION(:,:) :: zcpx, zcpy   ! Wetting/Dying gravity filter coef.
...
#endif
!AW v421 update         IF( ln_wd_il )   CALL wad_lmt_bt(zhU, zhV, sshn_e, ssh_frc, rDt_e)    !!gm wad_lmt_bt use of lbc_lnk on zhU, zhV
...
!jth implicit bottom friction:
      ...
      !
!AW v421 update      IF( ln_wd_il )   DEALLOCATE( zcpx, zcpy )
...
!AW end
   ...
   SUBROUTINE wad_tmsk( pssh, ptmsk )
      !!----------------------------------------------------------------------
      !!           !AW v420   ***  ROUTINE wad_lmt  ***
      !!               v421   ***  ROUTINE wad_tmsk  ***
      !!                    
