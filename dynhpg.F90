...
!AW v421 update      LOGICAL  ::   ll_tmp1, ll_tmp2               ! local logical variables
      REAL(wp), DIMENSION(A2D(nn_hls),jpk)  ::   zhpi, zhpj
!AW v421 update      REAL(wp), DIMENSION(:,:), ALLOCATABLE ::   zcpx, zcpy   !W/D pressure filter
      !!----------------------------------------------------------------------
      !
!AW v421 update      IF( ln_wd_il ) ALLOCATE(zcpx(A2D(nn_hls)), zcpy(A2D(nn_hls)))
      !
      IF( .NOT. l_istiled .OR. ntile == 1 )  THEN                       ! Do only on the first tile
         IF( kt == nit000 ) THEN
            IF(lwp) WRITE(numout,*)
